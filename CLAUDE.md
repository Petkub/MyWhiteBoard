...
- **Snap-to-grid** (`state.snap`, localStorage `wb-snap`, "⌗ snap" chip in the shape AND select sub-bars): shape endpoints snap to the `SNAP_STEP=28` lattice (snap applied BEFORE shift-squareLock so locked squares stay on-grid); select-move quantizes the cumulative drag into 28px steps (`sel.acc`). Pen/highlighter ink never snaps.
